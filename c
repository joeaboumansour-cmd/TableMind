...
// 1. Add import for queueStockDecrementsForTransaction
if (!checkout.includes('queueStockDecrementsForTransaction')) {
  checkout = checkout.replace(
    'import { useCartStore } from "@/lib/stores/cartStore";',
    'import { useCartStore } from "@/lib/stores/cartStore";\nimport { queueStockDecrementsForTransaction } from "@/lib/db";'
  );
  console.log('Checkout: Added queueStockDecrementsForTransaction import');
}

// 2. Add offline banner at the top of the checkout page (before the header)
if (!checkout.includes('Offline Mode') || !checkout.includes('isOfflineBanner')) {
  // Add isOffline state
  ...
  // Add offline banner before the header
  checkout = checkout.replace(
...
// 3. Queue stock decrements when online (after API call succeeds)
...
// 4. Queue stock decrements when offline (after queueing the transaction)
if (!checkout.includes('Queue stock decrements for offline transaction')) {
  checkout = checkout.replace(
...
// 1. Add import for getCachedProducts
if (!products.includes('getCachedProducts')) {
  products = products.replace(
    'import { downloadCSV, productsToCSV } from "@/lib/csv/utils";',
    'import { downloadCSV, productsToCSV } from "@/lib/csv/utils";\nimport { getCachedProducts } from "@/lib/db";'
  );
  console.log('Products: Added getCachedProducts import');
}

// 2. Modify fetchProducts to load from cache when offline
